...
# order of features in Moses graph output: w u d lm tm

# STEP: parse input parameters (ini_path, moses_path, latticemert_path, devel_src_path, devel_ref_path, working_dir)
# STEP: read provided moses.ini and get feature weights
# TRAINING LOOP:
# STEP: launch moses with -osgx and provided devel source (save results in working dir)
# STEP: launch LatticeMERT with current lambdas and provided devel reference (save results in working dir)
# STEP: parse score and new lambdas
# STEP: check if training loop terminates
...
# uses ABBR names.
my $ABBR_FULL_MAP = "d=weight-d lm=weight-l tm=weight-t w=weight-w g=weight-generation lex=weight-lex u=weight-u";
...
my $mert_mert_cmd = "$mertdir/LatticeMERT";
...
if (-e $ref_abs) {
  push @references, $ref_abs;
    ...
    die "Multiple reference files not yet supported by LatticeMERT";
...
while(1) {
  ...
  if (!$skip_decoder) {
      print "($run) run decoder to produce search graph\n";
      $nbest_file = run_decoder(\%used_triples, $PARAMETERS, $run, \@order_of_lambdas, $need_to_normalize);
      $need_to_normalize = 0;

      # archive the search lattice
      #safesystem("gzip -f $nbest_file") or die "Failed to gzip run*.sg";
...
#  $cmd = $cmd." --ifile run$run.$weights_in_file";
  ...
  $cmd = $cmd." $mert_mert_args";

  $cmd = $cmd." $nbest_file"; 

  print "$cmd\n";
#  die "Debug: stop at this point";
...
sub run_decoder {
    ...
    my $decoder_cmd;
      ...
      $decoder_cmd = "$___DECODER $parameters  -config $___CONFIG -inputtype $___INPUTTYPE $decoder_config -osgx $filename -i $___DEV_F > run$run.out";
