...
//    a.sort(Line::CompareBySlope);
  ...
  while (it != a.end())
  {
    if (it != a.begin())
    {
      ...
      while (true)
      {
        it->leftBound = (it->offset - it_prev->offset)
            / (it_prev->slope - it->slope);
        if (it->leftBound > it_prev->leftBound)
          break;
...
void sweepLine(vector<Line> &a)
{
  ...
  for (size_t i = 0; i < K; i++)
  {
    ...
    if (j > 0)
    {
      ...
      {
        if (l.offset <= a[j - 1].offset)
          continue;
        j--;
      }
      while (j > 0)
      {
        l.leftBound = (l.offset - a[j - 1].offset) / (a[j - 1].slope - l.slope);
        if (l.leftBound > a[j - 1].leftBound)
          break;
        j--;
      }
      a[j] = l;
    ...
    if (j == 0)
      a[j].leftBound = -numeric_limits<double>::infinity();
    j++;
  }
  a.resize(j);
}
...
//            << " Out: " << v.out.size() << endl;
    ...
    for (size_t i = 0; i < v.in.size(); ++i)
    {
      ...
      {
        Lattice::Edge & edge = lattice.getEdge(edgekey);
        cout << "WARNING: In edge from " << edge.from << " contains zero lines!"
            << endl;
      }
    ...
    for (size_t i = 0; i < v.out.size(); ++i)
    {
      ...
      if (edge.scores.size() > 0)
      {
        ...
        double dot_dir = dotProduct(edge.scores, dir);
        double dot_lambda = dotProduct(edge.scores, lambda);
        for (list<Line>::iterator lit = lines.begin(); lit != lines.end();
            ++lit)
        {
          lit->slope += dot_dir;
          lit->offset += dot_lambda;
